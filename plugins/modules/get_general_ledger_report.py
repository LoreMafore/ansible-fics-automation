...
DOCUMENTATION = r"""
---
module: get_general_ledger_report

short_description: Calls the FICS Mortgage Accountant special services API to generate a document containing the General Ledger Report.
...
description:
    - Calls the FICS Mortgage Accountant special services API to create the General Ledger Report file at the specified destination.
...
- name: create file to send
  get_general_ledger_report:
    dest: /mnt/fics_deliq/IT/Backups/fics/general_ledger_report.pdf
...
msg:
    ...
    type: str
    returned: always
    sample: '"Wrote files to /mnt/fics_deliq/IT/Backups/fics/general_ledger_report.pdf"'
changed:
...
def get_general_ledger_report(
    ...
    params: dict = {
        "Request":{
            "BeginningDate": get_start_date(),
		    "EndingDate": get_end_date(),
            "IsSortByChecked": False,
            "Token": api_token,
        }
    }
...
def run_module():
    ...
    trial_resp: dict = get_general_ledger_report(
        api_url=api_url, 
        api_token=api_token, 
